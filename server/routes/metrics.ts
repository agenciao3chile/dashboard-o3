...
export default async function metricsRoutes(app: FastifyInstance) {
  ...
  app.get("/api/flow", async () => {
    ...
    const [rew] = await rows(
      `SELECT COUNT(DISTINCT clave) AS tareas, COUNT(*) AS eventos FROM o3_rework_events`
    );
    const [ent] = await rows(`SELECT COUNT(*) AS c FROM o3_task_events WHERE estado = 'en_revision'`);
  ...
  app.get("/api/alerts", async () => {
    const [bloq] = await rows(`SELECT COUNT(*) c FROM o3_task_aging WHERE estado = 'bloqueado'`);
    const [sinmov] = await rows(`SELECT COUNT(*) c FROM o3_task_aging WHERE dias_sin_mov >= 3`);
    const [rev] = await rows(
      `SELECT COUNT(*) c FROM o3_review_spans WHERE en_curso AND dias_en_revision >= 2`
    );
    const [norep] = await rows(
    ...
    const conc = await rows(
      `SELECT ${CLIENTE_EXPR} AS cliente, COUNT(*) c FROM tareas_pendientes GROUP BY 1 ORDER BY c DESC LIMIT 1`
    );
    const [tot] = await rows(`SELECT COUNT(*) c FROM tareas_pendientes`);
